...
- Description
    ...
    This program calculates the Yokoi connectivity number for a downsampled version of lena.bmp. It begins by loading the image in grayscale and binarizing it: pixels with values greater than 127 are set to 255, while others are set to 0. The image is then downsampled to 64x64 by taking the top-left pixel from each 8x8 block.
    
    The function `num` is defined to classify connectivity relations, returning "q," "r," or "s" based on the relationships of neighboring pixels. The main function `YokoiConnectivityNumber` uses these classifications for each pixel and its neighbors, computing the connectivity number according to 4-connectivity. Specifically, a pixel is assigned a value of 5 if all neighboring regions are classified as "r"; otherwise, it receives the count of "q" classifications among its neighbors.
    
    The result is stored in a 64x64 matrix where only foreground pixels are processed. Finally, the connectivity numbers are saved in `result.txt` as a 64x64 matrix.
...
- Result
    ...
    <img src="./output/result.png" width=100%></img>
